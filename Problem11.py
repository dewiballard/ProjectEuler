...
# -*- coding: utf-8 -*-
"""
Created on Sat Nov 16 20:46:15 2019
...
for i in range(20):
    for j in range(16):
        # right/left products
        value = M[i][j]*M[i][j+1]*M[i][j+2]*M[i][j+3]
        if value > max_value: max_value = value
        # up/down products
        value = M[j][i]*M[j+1][i]*M[j+2][i]*M[j+3][i]
        if value > max_value: max_value = value

for i in range(16):
    for j in range(16):
        # right to left diagonal
        value = M[i][j]*M[i+1][j+1]*M[i+2][j+2]*M[i+3][j+3]
        if value > max_value: max_value = value

for i in range(3,20):
    for j in range(16):
        value= M[i-3][j+3]*M[i-2][j+2]*M[i-1][j+1]*M[i][j]
        if value > max_value: max_value = value
